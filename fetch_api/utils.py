...
from papers_search.constants import COUNTRIES, JURISDICTIONS, DATASOURCE
...
def filter_nodes(node_type, search_text, country, jurisdiction, source_id):
    ...
    filter_params = {"countries__icontains": country, "sourceID__icontains": source_id}
    ...
    node_set.filter(**filter_params)

    return node_set


"""
This are the parameters for filtering before a count is done.
    {
        'node_type': '',
        'name': '',
        'country': '',
...
def fetch_node_details(node_info):
    ...
    node = MODEL_ENTITIES[node_type].nodes.get_or_none(node_id=node_id)
